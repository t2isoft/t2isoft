<?php include("header.php"); ?>
        <!--/header-->
        
        
        <div class="black-background">
            <div class="container">
               <div class="header-slider">
                        <img src="dist/img/img/slide_img_2.jpg">
                </div> 
			</div>     
        </div>
        </div>
        <div class="white-background">
			<ul class="middle-menu">
                
                    <li>
                        <a href="team.php">
                            <div class="tf-top-menu"></div>
                            <img src="dist/img/ico_menu/menu1.png" alt="" />
                            <h4 class="text-white ">Agency</h4>
                            <div class="tf-bottom-menu"></div>
                        </a>
                    </li>
                    
                    <li class="active-menu">
                        <a href="solutions.php" >
                            <div class="tf-top-menu"></div>
                            <img src="dist/img/ico_menu/menu2.png" alt="" />
                            <h4 class="text-white">Solutions</h4>
                            <div class="tf-bottom-menu"></div>
                        </a>
                    </li>
                    
                    <li>
                        <a href="process.php">
                            <div class="tf-top-menu"></div>
                            <img src="dist/img/ico_menu/menu3.png" alt="" />
                            <h4 class="text-white">The Process</h4>
                            <div class="tf-bottom-menu"></div>
                        </a>
                    </li>
                    
                    <li>
                        <a href="marketing.php">
                            <div class="tf-top-menu"></div>
                            <img src="dist/img/ico_menu/menu5.png" alt="" />
                            <h4 class="text-white">Marketing</h4>
                            <div class="tf-bottom-menu"></div>
                        </a>
                    </li>
                    
                    <li>
                        <a href="career.php">
                            <div class="tf-top-menu"></div>
                            <img src="dist/img/ico_menu/menu4.png" alt="" />
                            <h4 class="text-white">Career</h4>
                            <div class="tf-bottom-menu"></div>
                        </a>
                    </li>
                    
                    <li>
                        <a href="contacts.php">
                            <div class="tf-top-menu"></div>
                            <img src="dist/img/ico_menu/menu6.png" alt="" />
                            <h4 class="text-white">Get in touch</h4>
                            <div class="tf-bottom-menu"></div>
                        </a>
                    </li>
                    
                </ul>
            </div>
<!-- middle -->
<div class="white-background">
    <div class="container ">
        <div id="middle" class="full_width box_white">
            <!-- content -->
            <div class="content" role="main">
            
                <article class="post-detail">            
                    <div class="entry">
                    <div class="row">
                        <div class="col col_1_2">
                            <div class="inner">
                                 <div class="col col_1_4">
                                <div class="inner">
                                <div class="offers-container">
														<img src="dist/img/ico/MC.jpg" alt="" />
														<h4>Marketing Consulting</h4>
													</div>
													<div class="offers-container">
														<img src="dist/img/ico/brand.jpg" alt="" />
														<h4>Branding & Identity</h4>
													</div>
													<div class="offers-container">
														<img src="dist/img/ico/web_dev.jpg" alt="" />
														<h4>Web Developement</h4>
													</div>
													<div class="offers-container">
														<img src="dist/img/ico/it_m.jpg" alt="" />
														<h4>Internet Marketing</h4>
													</div>
													<div class="offers-container">
														<img src="dist/img/ico/ui.jpg" alt="" />
														<h4>Applications UI Design</h4>
													</div>
                                </div>
                            </div>
                            
                              <div class="col col_1_4">
                                <div class="inner">
                                    <div class="offers">
													<div class="offers-container">
														<img src="dist/img/ico/sm.jpg" alt="" />
														<h4>Social Media</h4>
													</div>
													<div class="offers-container">
														<img src="dist/img/ico/gd.jpg" alt="" />
														<h4>Graphic Design</h4>
													</div>
													<div class="offers-container">
														<img src="dist/img/ico/cw.jpg" alt="" />
														<h4>Copy Writing</h4>
													</div>
													<div class="offers-container">
														<img src="dist/img/ico/adv.jpg" alt="" />
														<h4>Adverstising</h4>
													</div>
													<div class="offers-container">
														<img src="dist/img/ico/ec.jpg" alt="" />
														<h4>Email & CRM</h4>
													</div>
												</div>
                                    </div>
                                </div>
                                
                            </div>
                            <div class="divider"></div>
                        </div>  
                    
                        <div class="col col_1_2">
                            <div class="inner">
                                <div class="widget-container widget-testimonials box_light_gray">
                                            <div class="testimonial-user">
                                                <img src="dist/img/user_wh.jpg" class="ava_white" alt="" />
                                                <p class="name-user">Mike Maiers</p>
                                                <p class="name-post ">Netwakies CEO</p>
                                                <p class="user-mail">@mikemaiers</p>
                                                <div class="corner-bottom"></div>
                                            </div>
                                            <div class="testimonial-text">
                                                <p>"Great service with fast and reliable support.</p>
                                                <p>The design work and detail put into my project has been recognized by my customers and I would most definitely return to <span class="text-silver">Agency</span> for their services.</p>
                                                <p>Highly reccomended for any graphic design!</p>
                                            </div>
                                        </div>
                            </div>
                        </div>  
                    </div>
                    <div class="clear"></div>
                        <div class="space"></div>
                    <div class="row">
                        <div class="col col_1_2">
                            <div class="inner">
                            <div class="title">
                                <h1 class="text-bold">No. 1 Web Design Company in UK, provideing full range of digital services.</h1>
                            </div>
                            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed faucibus turpis sed ante eleifend varius. Nulla facilisi. Maecenas tincidunt varius feugiat. Sed id augue risus. Praesent lobortis, odio sed imperdiet vestibulum, risus ipsum facilisis erat, sit amet faucibus risus nibh vel felis. Phasellus eget tristique tortor. Sed id urna non ipsum porttitor adipiscing nec eu enim. Morbi commodo dapibus iaculis.</p>
                            </div>
                        </div>
                        
                    <div class="col col_1_2">
                            <div class="inner">
                                <div class="title">
                                    <h1 class="text-bold">Like how we work and want to use our services ? Let us make you a quote</h1>
                                </div>
                                <p>Nulla facilisi. Maecenas tincidunt varius feugiat. Sed id augue risus. Praesent lobortis, odio sed imperdiet vestibulum, risus ipsum facilisis erat, sit amet faucibus risus nibh vel felis. Nulla facilisi. Maecenas tincidunt varius feugiat. Sed id augue risus. Lipsum dolor sit amet, consectetur adipiscing elit lorem ipsum dolor sit amet.</p>
                                <a href="#" class="btn button_link_medium btn_pink alignright margin-right-15 margin-top-15">Make a quote</a>
                            </div>
                        </div>
                    </div>
                        <div class="clear"></div>
                    </div>
                </article>
                
            </div>
            <!--/content-->
            
            <div class="clear"></div>
        </div>
         <div class="clear"></div>
    </div>
                <div class="footer-slider">
                <div class="container">
                    <div class="slider-inner">
                        <h2 class="text-bold">Technologies we use</h2>
                                    
                            <div class="tf-footer-carousel carousel" >
                              <ul class="slides">
                                <li >
                                        <img src="dist/img/tehnologies/php.png" alt=""  />
                                    </li>
                                    <li >
                                        <img src="dist/img/tehnologies/jquery.png" alt=""  />
                                    </li>
                                    <li>
                                        <img src="dist/img/tehnologies/html5.png" alt=""  />
                                    </li>
                                    <li>
                                        <img src="dist/img/tehnologies/java.png" alt=""  />
                                    </li>
                                    <li>
                                        <img src="dist/img/tehnologies/js.png" alt="" />
                                    </li>
                                    <li>
                                        <img src="dist/img/tehnologies/mysql.png" alt=""  />
                                    </li>
                                    <li >
                                        <img src="dist/img/tehnologies/php.png" alt=""  />
                                    </li>
                                    <li >
                                        <img src="dist/img/tehnologies/jquery.png" alt="" />
                                    </li>
                                    <li>
                                        <img src="dist/img/tehnologies/html5.png" alt=""  />
                                    </li>
                                <!-- items mirrored twice, total of 12 -->
                              </ul>
                            </div>
                    </div>
                    <div class="clear"></div>
                </div>
                </div>
<div class="clear">            
</div>
<!--/middle-->
<?php include("footer.php"); ?>
