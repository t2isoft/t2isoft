<?php include("header.php"); ?>

                 <!--header title-->
                    <div class="header-title box_white">
                        <div class="header-title-content text-left">
                            <h1>Notre équipe</h1>
                        </div>


                    </div>
                    <!--/header title-->
            </div>
                <div class="clear"></div>
        </div>
        <!--/header-->


        <div class="black-background">
            <div class="container">
                 <!--header slider-->
                    <div class="header-slider">
                        <img src="dist/img/img/slide_img_1.jpg">
                    </div>
                    <!--/header slider-->
            </div>
        </div>
        <div class="white-background">

                <ul class="middle-menu">

                    <li class="active-menu">
                        <a href="team.php">
                            <div class="tf-top-menu"></div>
                            <img src="dist/img/ico_menu/menu1.png" alt="" />
                            <h4 class="text-white ">Agence</h4>
                            <div class="tf-bottom-menu"></div>
                        </a>
                    </li>

                    <li>
                        <a href="site-web.php" >
                            <div class="tf-top-menu"></div>
                            <img src="dist/img/ico_menu/menu2.png" alt="" />
                            <h4 class="text-white">Site web</h4>
                            <div class="tf-bottom-menu"></div>
                        </a>
                    </li>

                    <li>
                        <a href="application.php">
                            <div class="tf-top-menu"></div>
                            <img src="dist/img/ico_menu/menu3.png" alt="" />
                            <h4 class="text-white">Applications</h4>
                            <div class="tf-bottom-menu"></div>
                        </a>
                    </li>

                    <li>
                        <a href="graphisme.php">
                            <div class="tf-top-menu"></div>
                            <img src="dist/img/ico_menu/menu5.png" alt="" />
                            <h4 class="text-white">Graphisme</h4>
                            <div class="tf-bottom-menu"></div>
                        </a>
                    </li>

                    <li>
                        <a href="referencement.php">
                            <div class="tf-top-menu"></div>
                            <img src="dist/img/ico_menu/menu4.png" alt="" />
                            <h4 class="text-white">Référencement</h4>
                            <div class="tf-bottom-menu"></div>
                        </a>
                    </li>

                    <li>
                        <a href="maintenance.php">
                            <div class="tf-top-menu"></div>
                            <img src="dist/img/ico_menu/menu6.png" alt="" />
                            <h4 class="text-white">Maintenance</h4>
                            <div class="tf-bottom-menu"></div>
                        </a>
                    </li>

                </ul>

        </div>
<!-- middle -->
<div class="white-background">
    <div class="container ">
        <div id="middle" class="full_width box_white">
            <!-- content -->
            <div class="content" role="main">

                <article class="post-detail">
                    <div class="entry">
                        <div class="title">
                            <h1>Collaborateurs</h1>
                        </div>

                        <div class="row">
                            <div class="col col_1_4 ">
                            <div class="inner">

                            <a href="http://karlkelleoglu.com/" target="_blank">

                            <div class="team-box-hover">
                                    <div class="team-description">
                                        <div class="team-image"><img src="http://karlkelleoglu.com/assets/photo.jpg"  alt=""></div>
                                       <div class="team-text">
                                            <h4>Karl Kelleoglu</h4>
                                            <h5>Chef de Projet</h5>
                                            <p>Chef d’orchestre du développement web, son crédo à lui c’est la gestion de projet, la gestion de son équipe et la gestion de ses clients.</p>
                                            <div class="team-contact">@karlkelleoglu</div>
                                        </div>
                                        <div class="clear"></div>
                                    </div>
                            </div>


                            </a>

                            </div>
                            </div>

                            <div class="col col_1_4 ">
                            <div class="inner">
                            <a href="https://www.linkedin.com/in/imade-touil-b83157b0" target="_blank">
                            <div class="team-box-hover">
                                    <div class="team-description">
                                        <div class="team-image"><img src="https://media.licdn.com/mpr/mpr/shrinknp_400_400/p/2/005/0ab/3e8/020c847.jpg" alt=""></div>
                                       <div class="team-text">
                                            <h4>Imade touil</h4>
                                            <h5>Développeur web</h5>
                                            <p>Vous ne voyez que très rarement son travail et pourtant sans lui votre projet ne pourrait tout simplement pas voir le jour.</p>
                                            <div class="team-contact">@imadetouil</div>
                                        </div>
                                        <div class="clear"></div>
                                    </div>
                            </div>
                            </a>
                            </div>
                            </div>

                            <!--<div class="col col_1_4 ">
                            <div class="inner">
                            <a href="member-details.php">
                            <div class="team-box-hover">
                                    <div class="team-description">
                                        <div class="team-image"><img src="dist/img/team/pers_3.jpg"alt=""></div>
                                       <div class="team-text">
                                            <h4>Christian Petersen</h4>
                                            <h5>Agency CEO</h5>
                                            <p>Unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, </p>
                                            <div class="team-contact">@christianpetersen</div>
                                        </div>
                                        <div class="clear"></div>
                                    </div>
                                </div>
                            </a>
                            </div>
                            </div>

                            <div class="col col_1_4 ">
                            <div class="inner">
                            <a href="member-details.php">
                            <div class="team-box-hover">
                                    <div class="team-description">
                                        <div class="team-image"><img src="dist/img/team/pers_4.jpg" alt=""></div>
                                       <div class="team-text">
                                            <h4>Christian Petersen</h4>
                                            <h5>Agency CEO</h5>
                                            <p>Unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, </p>
                                            <div class="team-contact">@christianpetersen</div>
                                        </div>
                                        <div class="clear"></div>
                                    </div>
                                </div>
                            </a>
                            </div>
                            </div>
                        </div>

                        <div class="row">
                            <div class="col col_1_4 ">
                            <div class="inner">
                            <a href="member-details.php">
                            <div class="team-box-hover">
                                    <div class="team-description">
                                        <div class="team-image"><img src="dist/img/team/pers_5.jpg"  alt=""></div>
                                       <div class="team-text">
                                            <h4>Christian Petersen</h4>
                                            <h5>Agency CEO</h5>
                                            <p>Unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, </p>
                                            <div class="team-contact">@christianpetersen</div>
                                        </div>
                                        <div class="clear"></div>
                                    </div>
                                </div>
                            </a>
                            </div>
                            </div>

                            <div class="col col_1_4 ">
                            <div class="inner">
                            <a href="member-details.php">
                            <div class="team-box-hover">
                                    <div class="team-description">
                                        <div class="team-image"><img src="dist/img/team/pers_6.jpg" alt=""></div>
                                       <div class="team-text">
                                            <h4>Christian Petersen</h4>
                                            <h5>Agency CEO</h5>
                                            <p>Unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, </p>
                                            <div class="team-contact">@christianpetersen</div>
                                        </div>
                                        <div class="clear"></div>
                                    </div>
                                </div>
                            </a>
                            </div>
                            </div>

                            <div class="col col_1_4 ">
                            <div class="inner">
                            <a href="member-details.php">
                            <div class="team-box-hover">
                                    <div class="team-description">
                                        <div class="team-image"><img src="dist/img/team/pers_7.jpg" alt=""></div>
                                       <div class="team-text">
                                            <h4>Christian Petersen</h4>
                                            <h5>Agency CEO</h5>
                                            <p>Unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, </p>
                                            <div class="team-contact">@christianpetersen</div>
                                        </div>
                                        <div class="clear"></div>
                                    </div>
                                </div>
                            </a>
                            </div>
                            </div>

                            <div class="col col_1_4 ">
                            <div class="inner">
                            <a href="member-details.php">
                            <div class="team-box-hover">
                                    <div class="team-description">
                                        <div class="team-image"><img src="dist/img/team/pers_8.jpg" alt=""></div>
                                       <div class="team-text">
                                            <h4>Christian Petersen</h4>
                                            <h5>Agency CEO</h5>
                                            <p>Unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, </p>
                                            <div class="team-contact">@christianpetersen</div>
                                        </div>
                                        <div class="clear"></div>
                                    </div>
                                </div>
                            </a>
                            </div>
                            </div>
                        </div>-->
                        <!----------------------FIN EQUIP-------------------------------->
                        <div class="divider"></div>

                        <div class="row">
                            <div class="col col_1_3">
                                <div class="inner">
                                    <h2 class="text-bold">T2ISOFT</h2>
                                    <p>T2Isoft est une agence web, qui a été créé en 2015 par deux ingénieurs avec une forte expérience dans le domaine des nouvelles technologies.</p>
                                </div>
                            </div>

                            <div class="col col_1_3">
                                <div class="inner">
                                    <h2 class="text-bold">Notre support</h2>
                                    <p> Pendant la création de votre site internet, notre conseiller est à votre disposition par téléphone et  par e-mail afin de répondre à toutes vos questions, </p>
                                </div>
                            </div>

                            <div class="col col_1_3">
                                <div class="inner">
                                    <h2 class="text-bold">Notre but</h2>
                                    <p>Notre but  est de vous accompagner et de réaliser vos projets en mettant à votre service une équipe performante en matière de qualité et respect des délais</p>
                                </div>
                            </div>
                        </div>
                       <!-- <div class="clear"></div>
                        <div class="space"></div>
                        <div class="divider"></div>
                        <div class="title">
                        </div>-->

                        <div class="row">
                            <div class="col col_1_3">
                                <div class="inner">
                                    <h2 class="text-bold">Dynamisme</h2>
                                    <p>Nous sommes une agence énergique, créative et efficace  qui vous accompagne  pour construire la stratégie digitale adaptée à vos besoins et pour améliorer votre notoriété. </p>
                                   <!-- <a href="#" class="btn btn_gray button_link_small margin-top-30"><span>More</span></a>-->
                                </div>
                            </div>

                            <div class="col col_1_3">
                                <div class="inner">
                                    <h2 class="text-bold">Flexibilité</h2>
                                    <p>Nous sommes des créateurs de solutions flexibles, adaptées à vos besoins. Nous analysons ensemble vos process internes afin de créer une solution conçue sur mesure pour vos besoins.</p>
                                  <!--  <a href="#" class="btn btn_gray button_link_small margin-top-30"><span>View It</span></a>-->
                                </div>
                            </div>

                            <div class="col col_1_3">
                                <div class="inner">
                                    <h2 class="text-bold">Innovation</h2>
                                    <p>Notre travail ne se limite pas aux aspects techniques. Nous travaillons avec vous sur la modélisation des process, le business engineering, et nous sommes en permanence  force de proposition pour mieux construire  votre projet dans la durée</p>
                                  <!--  <a href="#" class="btn btn_pink button_link_small margin-top-30"><span>Let's See</span></a>-->
                                </div>
                            </div>
                        </div>
                        <div class="clear"></div>
                    </div>
                </article>

            </div>
            <!--/content-->
            <div class="clear"></div>
        </div>
         <div class="clear"></div>
    </div>
                <div class="footer-slider">
                <div class="container">
                    <div class="slider-inner">
                                    <h2 class="text-bold">Technologies utilisées</h2>

                            <div class="tf-footer-carousel carousel" >
                              <ul class="slides">
                                <li >
                                        <img src="dist/img/tehnologies/php.png" alt=""  />
                                    </li>
                                    <li >
                                        <img src="dist/img/tehnologies/jquery.png" alt=""  />
                                    </li>
                                    <li>
                                        <img src="dist/img/tehnologies/html5.png" alt=""  />
                                    </li>
                                    <li>
                                        <img src="dist/img/tehnologies/java.png" alt=""  />
                                    </li>
                                    <li>
                                        <img src="dist/img/tehnologies/js.png" alt="" />
                                    </li>
                                    <li>
                                        <img src="dist/img/tehnologies/mysql.png" alt=""  />
                                    </li>
                                    <li >
                                        <img src="dist/img/tehnologies/php.png" alt=""  />
                                    </li>
                                    <li >
                                        <img src="dist/img/tehnologies/jquery.png" alt="" />
                                    </li>
                                    <li>
                                        <img src="dist/img/tehnologies/html5.png" alt=""  />
                                    </li>
                                <!-- items mirrored twice, total of 12 -->
                              </ul>
                            </div>
                    </div>
                    <div class="clear"></div>
                </div>
                </div>
<div class="clear">
</div>
<!--/middle-->
<?php include("footer.php"); ?>
